...
**ステップ3: 発明原理の適用**
TRIZの40の発明原理からこの課題に最も適した原理を3〜5個選び、以下のルールを必ず守って表形式で出力してください。

❗️ **表の出力ルール（必須）**：
- 必ず4列構成で出力すること：「原理番号」「原理名」「内容（簡潔に30字以内）」「本アイデアへの適用例（50字以内）」
- 内容は簡潔に　7c「|」区切り文字はセル内に含めないこと
- 必ず1行1原理で改行すること
- 表のフォーマット例：
| 原理番号 | 原理名 | 内容 | 適用例 |
|------|------|------|------|
| 1 | 分割 | 物体を部分に分ける | センサーを分散配置 |
| 2 | 分離 | 不要部分を除去 | 不要機能を別モジュール化 |
実際の出力は上記のフォーマットに従って、選んだ原理に置き換えて出力してください。
...
- 各フェーズで想定される主なリスクと対応策も記載\n\n`;
  ...
  if (items.includes('競合製品分析')) {
    p += `### 🏭 5. 競合製品分析
- 国内外の競吃品・代替品を3〜5件列挙（製品名・企業・特徴・価格帯）
- 本アイデアの差別化ポイント（競健比較表）\n\n`;
  }
